 ...
 *
 * Hub for generating bi-weekly hourly invoices from logged hours.
 * Flow:
 *   1. User taps "Generate invoice" → wizard opens at step 1
 *   2. Step 1: pick a date range on the calendar
 *   3. Step 2: pick a client (or create one via ClientEditSheet)
 *   4. Step 3: confirm hourly rate / tax / due date / notes,
 *              tap "Generate" → invoice row created, PDF rendered,
 *              OS share sheet opens automatically
 *   5. Saved invoices appear in the list below the header; tap one
 *      to view a read-only summary and re-share the PDF.
 *
 * The full invoice/edit features from onsite-timekeeper (inline day
 * edit, rate override, regenerate-PDF, swipe-to-delete) are not
 * surfaced in this MVP — adding them is a follow-up phase.
 */
...
function HourlyWizard({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  // Load days when range completes
  useEffect(() => {
  ...
  const handleGenerate = async () => {
    ...
    // Persist client (upsert by name) so it shows up in suggestions next time
...
// Suppress unused warning for isFutureDay (re-exported for clarity / future use)
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const _unused = isFutureDay;
